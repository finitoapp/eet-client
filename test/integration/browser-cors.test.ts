 ...
 * inspected regardless of outcome) and asserts the endpoint DOES grant cross-origin access. The
 * endpoint now sends `Access-Control-Allow-Origin: *` and allows `POST`, so `submit()` can be
 * called directly from browser JavaScript on any origin without a backend/proxy in between. A
 * failing assertion here would mean the server stopped granting that access.
 *
...
(ENABLED ? describe : describe.skip)("Live EET playground CORS behavior (opt-in, network)", () => {
  test("grants an arbitrary browser origin permission to call the SOAP endpoint", {
    timeout: 15_000,
  }, async () => {
    ...
    // A real browser only sends the actual POST once the preflight grants both of these.
    ...
    assert.strictEqual(grantsArbitraryOrigin, true);
    assert.strictEqual(grantsPost, true);
  });
});
